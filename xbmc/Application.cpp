...
void CApplication::FatalErrorHandler(bool WindowSystemInitialized, bool MapDrives, bool InitNetwork)
{
  ...
  fprintf(stderr, "Error log at %splex.log\n", g_settings.m_logFolder.c_str());
  abort();
}
...
bool CApplication::Create()
{
  ...
  CLog::Log(LOGNOTICE, "Starting Plex, Platform: Mac OS X (%s). Built on %s (Git:%s)", Cocoa_GetMachinePlatformVersion().c_str(), __DATE__, GIT_REV);
#elif defined(_LINUX)
  ...
  CLog::Log(LOGNOTICE, "Log File is located: %sPlex.log", g_settings.m_logFolder.c_str());
  CLog::Log(LOGNOTICE, "-----------------------------------------------------------------------");
...
bool CApplication::OnMessage(CGUIMessage& message)
{
  ...
  case GUI_MSG_QUEUE_NEXT_ITEM:
    {
     // check if playing a stack, in that case don't
     // queue anything (due no proper handling for stacks)
